...
def fetch_difumo(dimension=64, resolution_mm=2, data_dir=None):
    ...
    data_dir = get_dataset_dir(data_dir=data_dir, dataset_name=dataset_name,
                                verbose=1)

    # Download the zip file, first
    files = fetch_files(data_dir, files, verbose=2)
    ...
    if not os.path.exists(os.path.join(data_dir, 'README.md')):
        fetch_files(data_dir, readme_files, verbose=2)
